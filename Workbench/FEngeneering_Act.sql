...
CREATE TABLE IF NOT EXISTS `mydb`.`assists_interaction_session` (
  ...
  `end_time` DATETIME GENERATED ALWAYS AS () VIRTUAL,
  `session_status` ENUM('active', 'completed', 'interrupted') GENERATED ALWAYS AS (),
  `interaction_type` ENUM('audio', 'text', 'video') NOT NULL,
  `status` ENUM('interrupted', 'completed', 'active') NOT NULL,
  `userid` INT NOT NULL,
...
-- -----------------------------------------------------
-- Table `mydb`.`assists_contact_info`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`assists_contact_info` (
